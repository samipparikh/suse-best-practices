...
grows, so does the need to manage and get the best out of the data. SAP Data Hub
2 is a tool that makes it easier to deal with large amounts of data. SUSE
Container as a Service Platform 3 (SUSE CaaS Platform 3) is the perfect
foundation for SAP Data Hub 2.
...
SAP's hardware requirements can be found in the link:https://help.sap.com/viewer/product/SAP_DATA_HUB/[SAP Data Hub Install Guide].
...
The minimum hardware requirements for on-premises installation of SAP Data Hub 2
are:

* 4 Kubernetes cluster nodes (one master node and three worker nodes)

** All nodes should have at least four CPU cores

** The master node should have > 32 GiB RAM

** The three worker nodes should have > 64 GiB RAM
...
It is not recommended to do the installation of SAP Data Hub Foundation from
within the SUSE CaaS Platform Cluster. You should use an external "jump host".

The hardware and operating system specifications for the this machine could, for
example, be as follows:

* SUSE Linux Enterprise Server 12 SP4 or 15 (or even openSUSE Leap 15.X)
* Two CPU cores
* 8 GiB RAM
* Disk space:
** 50 GiB for `/`, including the space for the SAP Data Hub 2 software
** At least 20 GiB for `/var/lib/docker` (necessary for the SAP Data Hub 2
installation)
...
** link:https://launchpad.support.sap.com/#/notes/2764652[SAP Data Hub 2.6 release notes]
** link:https://launchpad.support.sap.com/#/notes/2686169[Prerequisites for installing SAP Data Hub]
...
To receive maintenance updates for SUSE products, you need a valid subscriptions
for the products. For more information about subscriptions for SUSE CaaS
Platform, see https://www.suse.com/support/?id=SUSE_CaaS_Platform.


=== Read the Deployment Guide for SUSE CaaS Platform 3

SUSE CaaS Platform is designed to simplify the installation of Kubernetes. For
more information on this topic, see the SUSE CaaS Platform 3 _Deployment Guide_.

For further reference, a _Quick Start Guide_ and an _Administrator's Guide_ are
available as well.

All the product documentation can be downloaded from
https://documentation.suse.com/suse-caasp/3/


=== Installing SUSE CaaS Platform 3

This document describes the installation of SUSE CaaS Platform 3 from ISO images.
Make sure that the host names of the cluster nodes can be resolved via DNS. A
static network setup is preferred.
...
From the GRUB menu, choose _Installation_.

==== Installing the Administration Node
// Add Pictures

. After initializing the hardware, the YaST installer opens the network
...
.. Select the keyboard layout and language you need.

.. Specify your subscription credentials, or the URL of your SMT/RMT server to
register your product.

.. Set the password for the root account.

.. Assign the role _Administration Node_.
+
...
. Boot each machine from the ISO image and select _Installation_ from the GRUB boot menu.
